...
fn main() -> anyhow::Result<()> {
    ...
    let local_private_key = EphemeralPrivateKey::generate(&agreement::X25519, &rng).unwrap();
    let _local_public_key = local_private_key.compute_public_key().unwrap();
    ...
    let peer_public_key_bytes = {
        ...
        let peer_private_key =
            agreement::EphemeralPrivateKey::generate(&agreement::X25519, &rng)
                .map_err(|e| anyhow::format_err!("EphemeralPrivateKey::generate failed: {}", e))?;
        peer_private_key
            .compute_public_key()
            .map_err(|e| anyhow::format_err!("compute_public_key failed: {}", e))?
    };
    ...
    let _encrypted_result = encryptor
    ...
    let _decrypted_result = decryptor
        .decrypt(
            &mut write_buffer.take_read_buffer(),
            &mut write_buffer2,
            true,
        )
        .unwrap();
